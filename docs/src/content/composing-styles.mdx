...
In essence, all composers are just objects that are infinitely chainable and can output collected CSS rules.

This means they can be assigned to variables, passed as arguments, etc.

## Simple styles can be reused and specialized

```tsx
import { $flex } from 'stylings';

const flex = $flex.horizontal.alignCenter.gap(2);
